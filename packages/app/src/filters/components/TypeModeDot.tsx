...
import { colors, GearIcon, Popover } from "@boundless-grimoire/ui";
import { useFilterStore } from "../store";

type TypeMode = "or" | "and";

const MODES: { value: TypeMode; label: string; description: string }[] = [
  { value: "or", label: "OR", description: "Any selected type can match" },
  { value: "and", label: "AND", description: "All selected types must match (e.g. Artifact + Creature)" },
];
...
/**
 * Gear dot for the Type combine mode (OR / AND). Mirrors
 * CustomQueryModeDot; sits in the action slot of the Type FilterField.
 */
export function TypeModeDot() {
  const mode = (useFilterStore((s) => s.state.typeMode) ?? "or") as TypeMode;
